...
public class RentalHistoryDAO {
    ...
    public void returnBook(int bookNo) throws SQLException {
    ...
    private boolean isOverDueDate(Connection conn, Integer userNo) {
        try (PreparedStatement pstmt = conn.prepareStatement("SELECT ifnull(u.overdue_date, date_sub(now(), interval 1 day)) AS overdue_date FROM user as u where user_no = ?")) {
            pstmt.setInt(1, userNo);
